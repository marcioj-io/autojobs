...
import { RuntimeService } from '@autojobs/db';
...
export class RuntimeController {
    ...
    auditLogsService;
    normalizeModality(location) {
        const value = location.toLowerCase();
        if (value.includes('remote') ||
            value.includes('remoto')) {
            return 'Remoto';
        }
        if (value.includes('onsite') ||
            value.includes('presencial')) {
            return 'Presencial';
        }
        return 'Híbrido';
    }
    mapEngineJobToJobRecord(job, profile, score) {
        return {
            id: job.id,
            company: job.company,
            title: job.title,
            ...
            modality: this.normalizeModality(job.location),
    ...
    async execute(options) {
        ...
        try {
            ...
            for (const job of jobs) {
                ...
                const isAllowedHybridLocation = hybridCities.some((city) => loc.includes(city.toLowerCase()));
                ...
                const score = calculateScore({
                    ...
                    easyApply: job.easyApply,
                    positiveKeywords: [
                        ...JSON.parse(profile.searches),
                        ...JSON.parse(profile.keywords)
                    ],
                    negativeKeywords: JSON.parse(profile.negativeKeywords)
                });
                const mappedJob = this.mapEngineJobToJobRecord(job, profile, score);
                await this.persistence.persistJob(mappedJob);
            }
            ...
            pipelineResult.averageScore =
                jobs.length > 0
                    ? jobs.reduce((acc, j) => acc + (j.score ?? 0), 0) / jobs.length
                    : 0;
            ...
            await this.runtimeService.updateState(this.runtimeStateId, {
                ...
                health: this.healthService.determineHealth(recovery.nextState, state.consecutiveFailures + 1, state.sessionStatus),
