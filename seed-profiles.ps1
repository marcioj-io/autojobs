...
# === DEFINIÇÃO DO PROFILE (substituído pelo profile enviado) ===
$profile = @{
    ...
    targetRoles = @(
        ...
        "Frontend Developer","React Developer","desenvolvedor front-end","Desenvolvedor back-end",
        "back-end","front-end","backend","frontend","backend engineer","frontend engineer",
        "front-end engineer","back-end engineer"
    )
    targetAreas = @()  # vazio conforme enviado
    seniority = @("Pleno","Senior","Pleno/Senior")
    searchLocation = @("Brasil","São Paulo")
    allowedModalities = @("Remoto","Híbrido")
    hybridCities = @("sao paulo","osasco","brasil")
    skillMatrix = @{
    ...
    negativeKeywords = @("Java","Spring","PHP","Laravel","Ruby","Rails","Angular","Vue","Delphi","Cobol","Django","Flask","rust","WordPress","Account Manager","sdr","flutter","mobile")
    minScore = 65
    dailyLimit = 40
