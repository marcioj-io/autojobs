...
exports.parseEasyApplyForm = parseEasyApplyForm;
...
function normalizeFieldType(elementType, typeAttribute, label) {
    ...
    if (typeAttribute === 'checkbox')
        return normalizedLabel.includes('yes') || normalizedLabel.includes('no') ? 'yesno' : 'checkbox';
...
function guessRequired(element, label) {
    if (element.required)
        return true;
    const normalized = label.toLowerCase();
    return normalized.includes('required') || normalized.includes('*');
}
async function parseEasyApplyForm(page) {
    ...
    const fields = await page.$$eval(`${formSelector} ${FIELD_SELECTORS.join(', ')}`, (elements) => {
        return elements.map((element) => {
            ...
            const labelElement = wrapper?.querySelector('label, legend, span, p');
